...
| # | item | why it is real |
|---|---|---|
| B1 | **Re-verify `api-contracts.md`, `data-model.md`, `domain-model.md`, `lifecycle-model.md` in `docs/17-features/FP-015-self-service/` against the tree.** Report discrepancies only; the documents are the architect's to edit. One at a time, reporting each. | Landed 2026-08-30 **explicitly marked as never re-checked**. The one document that WAS checked lost four of eight sections in three days, so the base rate is high. |
