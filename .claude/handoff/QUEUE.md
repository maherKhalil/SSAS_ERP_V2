...
| # | item | status | detail |
|---|---|---|---|
| 131 | **Read the request DTOs: does each of the 19 Class-A codes correspond to ONE unambiguous serialized property, and is that correspondence mechanically derivable?** ⚠ **This decides design (b) AND supplies its guard** — a guard asserting *"each `Field` value matches a serialized property name on the corresponding request DTO"* needs exactly this mapping, and the coupling is unacceptable without it. ⚠ **If any of the 19 has no single property, that is not a detail: it is (b) failing PARTIALLY, which is worse than either design, because a client cannot then tell which codes carry a field.** Report and stop; the architect rules after. | **open — ruled** | BOARD 2026-08-30 |
