...
{
  public async Task<Result> HandleAsync(ReactivateTenantCommand command, CancellationToken cancellationToken = default)
  {
    ...
    var transition = tenant.Reactivate(command.Reason, actor.Value, Guid.NewGuid(), clock.UtcNow);
