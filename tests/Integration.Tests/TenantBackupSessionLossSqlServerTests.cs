...
// THE REASON IT WAS ASSUMED TO HAVE WAS NOT THE REASON IT HAD. It takes the production applock
// N'SSAS.TenantStorage.Backup', which reads as instance-wide. Traced 2026-08-23: `sp_getapplock` is
// DATABASE-SCOPED, and the worker connects with `-d` against the fixture's own disposable catalog
// (SSAS_ERP_BACKUPT_<guid>), so the lock lives in a per-test database and cannot contend with any other
// class. What remains is real backup I/O and an abrupt process kill — load, not sharing.
