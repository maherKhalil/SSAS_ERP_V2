...
| Attempt to modify a posted journal | **no route exists** — there is no `PUT`, `PATCH` or `DELETE` on `/journals/{journalEntryId}`, so a client meets a routing failure rather than a named error | `BR-GL-0002` |
