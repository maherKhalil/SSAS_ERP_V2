...
| # | item | status | detail |
|---|---|---|---|
| 127 | **Apply Principle 16e's naming to the load-bearing controls already found.** At minimum the Unicode third test and `The_owned_relationships_the_guard_covers_are_the_ones_the_model_declares` — **both protect a ban that does not mention them, and both read as tests of something else.** Name each for what it holds up. **Small, and it is the ONLY remedy for the category no instrument can measure.** | **open — ruled** | BOARD 2026-08-30 |
| 128 | **The 129-code `request.invalid` collapse, re-scoped by what 123 changed.** With `detail` now carrying the message, the collapse costs **machine-readability, not information** — a human sees the explanation. ⚠ **So the question is no longer "which of the 129 lose detail" but the much smaller "which must a caller BRANCH on rather than merely read".** Pagination was one and is fixed. **Report the candidates; propose nothing.** | **open — ruled** | BOARD 2026-08-30 |
