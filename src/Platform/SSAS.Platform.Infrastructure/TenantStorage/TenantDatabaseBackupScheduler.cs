  ...
  ILogger<TenantDatabaseBackupScheduler> logger) : ITenantDatabaseBackupScheduler
{
    ...
    CancellationToken cancellationToken = default)
  {
    ...
    // THE CURSOR RESETS EVERY SWEEP. A database that becomes due behind the cursor waits for the next pass,
    // which is a bounded and acceptable delay; the alternative — a persisted cursor — would carry state that
    // can drift out of step with a fleet that changes underneath it.
    var afterId = 0L;
    ...
    using var globalGate = new SemaphoreSlim(Math.Max(1, options.MaxConcurrentBackups));
    ...
    try
    {
      while (!cancellationToken.IsCancellationRequested)
      {
        var page = await fleetReads.ListBackupCandidatesAsync(
          afterId, Math.Max(1, options.BatchSize), cancellationToken);
        ...
        afterId = page[^1].TenantDatabaseId;
        summary.Eligible += page.Count;

        var due = SelectDue(page, clock.UtcNow);
        if (due.Count == 0)
        {
          continue;
        }

        var runnable = await SuppressRecentAttemptsAsync(due, cancellationToken);
    ...
    DateTimeOffset nowUtc)
  {
    ...
    foreach (var candidate in page)
    {
      ...
      {
        due.Add(new DueWork(candidate, operation));
      }
    }
    ...
    CancellationToken cancellationToken)
  {
    ...
    foreach (var work in due)
    {
      ...
      if (TenantDatabaseBackupRetryPolicy.ShouldSuppress(
        latest, nowUtc, options.FailureInitialBackoff, options.FailureMaximumBackoff))
      {
        continue;
      }
    ...
    CancellationToken cancellationToken)
  {
    ...
    {
      var gate = ServerGate(serverGates, work.Candidate.ServerKey, options.MaxConcurrentPerServer);
    ...
    CancellationToken cancellationToken)
  {
    ...
    try
    {
      ...
      try
      {
        ...
        // The scheduler passes an identifier and an operation. Everything authoritative — policy, hosting
        // mode, routing, destination, credentials — is re-read by the executor and provider at this point,
        // because the projection that selected this work is advisory and may already be stale.
        var outcome = await executor.ExecuteAsync(
          work.Candidate.TenantDatabaseId, work.Operation, cancellationToken);
...
#pragma warning restore CA1031
    ...
    int perServerLimit)
  {
    ...
    {
      gate = new SemaphoreSlim(Math.Max(1, perServerLimit));
  ...
  private sealed record DueWork(TenantDatabaseBackupDueCandidate Candidate, TenantDatabaseBackupOperation Operation);
