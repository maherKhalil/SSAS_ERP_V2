...
| # | item | status | detail |
|---|---|---|---|
| **204** | ⚠ **THE RESULTS TRAIL HAS AMBIGUOUS NUMBERS, AND A CITATION CANNOT BE RESOLVED WITHOUT KNOWING WHICH SERIES IS MEANT.** Independently re-derived: **137 distinct `T-` numbers across 139 FILES — so two numbers carry duplicate files — and 47 distinct `item-` numbers, with EIGHT numbers appearing in BOTH series: 156, 157, 158, 179, 180, 191, 194, 201.** **For each of the eight: do the two files describe the SAME item or DIFFERENT ones?** ⚠ **If different, *item 157* is genuinely ambiguous in this repository and every citation of a number in that range is unresolvable — which matters because this loop cites item numbers constantly.** **And name the two `T-` numbers carrying duplicate files, and say whether those are copies or distinct items.** **(b) `T-181`'s result file is ALIVE ON ITS OWN BRANCH and is the ONLY ONE OF THE 27 THAT CAN BE RECOVERED. Commit it to the trail, noting where it came from and that the other 26 have no history anywhere.** ⚠ **Report the ambiguity before proposing a renaming — I am not asking you to renumber anything, and a renaming would break every existing citation.** | **open — ruled** | BOARD 2026-08-31 |
