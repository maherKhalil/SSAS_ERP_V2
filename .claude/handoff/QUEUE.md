...
| # | item | status | detail |
|---|---|---|---|
| 126 | **Floors and controls for the 25 unguarded runtime-enumeration bans, across 14 files.** ⚠ **A shared `model.GetEntityTypes()` floor is approved for the ROOT — it catches a model that failed to build — and DOES NOT discharge the matcher control**, because each ban filters that walk differently and shares nothing else. **Per-predicate known-positive controls cannot be shared:** assert `IsMonetary` still selects real columns, assert the reference-FK filter still selects real foreign keys. **Template already in hand — `UnicodeStringPersistenceArchitectureTests`' third test is exactly that control**; copy its shape and give each copy a name stating what it holds up, per Principle 16e. | **open — ruled** | BOARD 2026-08-30 |
| 125 | **Pin the route precedence for the three literal/parameter pairs — do not restore the constraints.** `/api/hr/employees/export`, `/export-runs`, `/import-runs` each sit beside `/{employeeId}` at segment 3, same method, same group. **Before T-238 the `:guid` constraint refused `export` outright; now both templates match and ASP.NET Core's literal-beats-parameter precedence decides** — correct, stable, and documented nowhere in this repository. ⚠ **Assert the HANDLER, not just the status: a 200 proves the right route won.** Restoring the constraint would undo Principle 12 for these routes and bring back 404-for-malformed. | **open — ruled** | BOARD 2026-08-30 |
