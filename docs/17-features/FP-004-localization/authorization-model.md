...
|---|---|---|
| View localization resources | `Platform.Localization.View` | list/get/effective administrative views |
...
|---|---|---|---|
| ordinary public system-default group | anonymous allowed only for explicitly public group | none | no Tenant override/state |
...
| list/get/effective administration | authenticated | View | current Tenant, live Active |
...
Current FP-003 status is authoritative. Provisioning, Suspended, Archived, missing, or not-yet-trusted Tenant context denies management and Tenant-effective private groups. A JWT, permission, or populated localization cache cannot override live eligibility. Pre-authentication pages use public system defaults. Tenant switch clears previous localization state; suspension after cache population prevents reuse.
...
Authorization status and immutable technical code/type never localize. ResourceKey and optional safe title/detail are display aids; sensitive causes remain generic. Production management is feature-gated off until immutable-audit persistence, configured retention, and audit readiness/health succeed. Effective read-only resolution remains available. Version history is not the immutable administrative audit store.

## Required security verification

Focused tests cover positive/negative View, Manage, ViewHistory; Preview/Undo/Restore requiring Manage; history denied without ViewHistory; ordinary-user denial; no writable/forged TenantId; inactive Tenant; anonymous public-only groups; pre-selection denial of private Tenant-effective groups; cross-Tenant safe absence; and suspension after cache population.
