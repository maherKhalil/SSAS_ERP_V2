...
{
  public async Task<Result> HandleAsync(AssignPermissionToRoleCommand command, CancellationToken cancellationToken = default)
  {
    ...
    var domainResult = role.AssignPermission(permission, execution.Value.Actor, Guid.NewGuid(), clock.UtcNow);
