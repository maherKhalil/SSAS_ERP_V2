...
| `BR-SUB-0008` | **Platform-plane routes are never subject to module enablement** — authentication, tenant selection, refresh, logout, platform support and the subscription surface itself stay reachable | `ADR-017:169`, `:376`–`:378` (**amended by `DEC-L-024`**, formerly `ADR-021:207`); `REQ-SUB-0013` |
