...
{
  public async Task<Result> HandleAsync(ArchiveTenantCommand command, CancellationToken cancellationToken = default)
  {
    ...
    var transition = tenant.Archive(command.Reason, actor.Value, Guid.NewGuid(), clock.UtcNow);
