...
| **180** | ⚠ **NAME THE INVARIANT WHERE IT IS RELIED UPON — `AccountActionToken`'s FILTER IS CORRECT FOR A REASON THREE FILES AWAY.** The index `[Purpose, TenantId?, TenantUserId?]` is filtered on `[TenantUserId] IS NOT NULL` and never mentions `TenantId?`; it is safe only because the aggregate throws unless the binding is Invitation (both set) or PasswordReset (both null). ⚠ **A CHANGE TO THAT BINDING RULE WOULD MAKE THE INDEX WRONG SILENTLY — the filter would still exist and 177's guard would still pass.** **(a) ESTABLISH FIRST whether a test pins the binding invariant itself.** ⚠ **If nothing pins it, THAT is the finding and it outranks the rest of the item: pin it, and say so.** **(b) Then cite that test BY NAME at the index, and cite the index at the invariant** — Principle 16e in both directions: **a load-bearing invariant must say what it holds up, and the thing it holds up must say what it rests on.** ⚠ **(c) A COMMENT IS THE WEAKEST REMEDY THIS LOOP HAS AND YOU SHOULD SAY SO IN IT — the chain is: the TEST reddens on a binding change, and the COMMENT tells whoever sees it to look at the index.** **If you can see a cheap way to make the index itself redden, report it rather than building it.** **(d) And settle the `LeaveRequest` observation while you are there: its filter uses `'Submitted'` where every Platform filter uses `N'…'`. Say whether the column is `nvarchar` and whether the missing `N` prefix changes behaviour or is only inconsistency.** | **open — ruled** | BOARD 2026-08-31 |
