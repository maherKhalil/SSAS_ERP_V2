...
| # | item | status | detail |
|---|---|---|---|
| 120 | **Six structural guards pass over an empty type set — 34 test methods currently asserting nothing if their enumeration collapses.** `TenantBackupFoundation` (10), `TenantBackupProvider` (9), `TenantCutoverFreeze` (5), `DepartmentApi` (4), `PlatformSupportAdministrativeRecovery` (3), `PlatformSupportAuthenticationSurface` (3). **A floor per ENUMERATION, not per file.** ⚠ **Floor the set each test ITERATES — after the filter — not `GetTypes()`:** a root cannot vanish silently, a filter can, and a renamed namespace empties a predicate while every type still loads. **Plant each twice** — `Take(0)` for a dead enumeration, a broken predicate for a dead filter. **Order: the two backup files first (19 of 34, and the surface where a silent green has already cost twice).** Probe `ImportExportArchitectureTests` before calling the batch complete. | **open — ruled** | BOARD 2026-08-30 |
