...
| # | item | status | detail |
|---|---|---|---|
| 132 | **Implement design (b) — a `field` extension — scoped to Class A's 17 clean codes. Guard ships WITH the change.** `Error` gains `string? Field = null` (source-compatible: zero positional deconstructions), 17 constants gain a field name, `ApiError` propagates it as `Detail` already does, 11 mapper wrappers take `error` rather than `error.Message`, `Problem` adds one extensions entry. **Excluded: `EmployeeImport.EmploymentDateInvalid`** (already carries a column AND a row number; never becomes an `ApiError`) **and `Company.InvalidSelectionFormat`** (a header, not a body property — **do not widen a definition for a population of one**). ⚠ **The guard must assert BOTH directions: every mapped field names a real property on the named DTO, AND every `Error` with a non-null `Field` has a map row** — otherwise a code gains a field, nobody adds a row, and the guard goes green having checked nothing. | **open — ruled** | BOARD 2026-08-30 |
