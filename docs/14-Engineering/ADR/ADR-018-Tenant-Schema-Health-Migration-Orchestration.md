...
Both contexts live in `SSAS.Platform.Infrastructure`, which is also its own startup project **for applying migrations** (the API host does not reference `Microsoft.EntityFrameworkCore.Design`, so it cannot be a startup project at all).

**Applying and scaffolding do not take the same startup project for the tenant stream.** `database update` runs SQL already recorded in a migration and needs no model; `migrations add` **diffs a model** and therefore needs the complete one. `SSAS.Platform.Infrastructure`'s design-time factory builds the model Platform itself can see — its own tenant entities and nothing else — which is correct for applying and **wrong for scaffolding**. See *Scaffolding a tenant migration* below, and use it: the difference is not cosmetic.
...
## Scaffolding a tenant migration — a different startup project, and why

*Added 2026-08-26 (T-050). The procedure above was published for `database update` and was correct for it; **the same project pair was silently correct for one command and destructive for the neighbouring one**, and nothing said so. `DEC-L-049`.*

**`migrations add` against `TenantDbContext` must use `tools/SSAS.Tenant.MigrationTool` as its startup project:**

```
SSAS_TENANT_MIGRATION_SQLSERVER="<tenant database connection string>" \
dotnet ef migrations add <Name> \
  --project src/Platform/SSAS.Platform.Infrastructure/SSAS.Platform.Infrastructure.csproj \
  --startup-project tools/SSAS.Tenant.MigrationTool/SSAS.Tenant.MigrationTool.csproj \
  --context TenantDbContext
```

**Why.** A migration is generated by diffing the model against the snapshot, so the model has to be the **composed** one — every module's entities included. Only a project permitted to reference every module can compose it, and `ComposedTenantDbContextFactory` in that tool is where the four `ITenantModelContributor` implementations are named. `SSAS.Platform.Infrastructure` may not reference the modules and its factory therefore composes none of them.

### What goes wrong if the `database update` project pair is used to scaffold

**It does not fail. It produces a migration whose `Up` drops every module table** — 32 `DropTable` covering the whole of HR, Finance/GL, Payroll and Attendance, because from that model's point of view those tables exist in the database and in no model, which is the definition of a table to drop.

**EF's *"An operation was scaffolded that may result in the loss of data"* warning is emitted, and it is not sufficient.** The output is long and structurally ordinary; it reads as *a big migration, review carefully*, when the correct reading is **discard it entirely and re-run with the tool.** Volume and a warning together make destructive output look like legitimate work.

**Recognising it:** any tenant migration whose `Up` contains `DropTable` for a module table was scaffolded with the wrong startup project. **No committed tenant migration has ever contained one** — verified across all thirteen at the time this was written — and none should.

**Applying is unaffected.** `database update` above stays exactly as it is; it replays recorded operations and never consults the composed model.
