...
| **179** | ⚠ **IS ANY OF THE 16 EXPLICIT FILTERS WRONG RATHER THAN ABSENT? YOU NAMED THIS AND IT IS YOURS TO TAKE.** **A filter naming the wrong column passes 177's guard exactly as a correct one does** — the guard asks whether a filter EXISTS, never whether it is RIGHT. **That is the `AC-SUB-0020` shape: an assertion true of its subject and silent about the property anyone cares about.** **For each of the 16, read the filter expression from the MODEL (`IIndex.GetFilter()`) and check it against the index's own columns and their nullability.** ⚠ **Say for each whether it was WRITTEN or supplied BY CONVENTION — the convention-supplied ones cannot be wrong, so the population that can be is smaller than 16 and you should say how much smaller.** ⚠ **A wrong filter is a LIVE DATA DEFECT, not a guard gap: report it and stop before extending anything.** **If none is wrong, say whether the guard can cheaply be strengthened to check correspondence rather than presence — and if it cannot be done without re-parsing T-SQL, say THAT and do not attempt it.** | **open — ruled** | BOARD 2026-08-31 |
