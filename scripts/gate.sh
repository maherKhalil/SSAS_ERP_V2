...
if [ $GATE_FAILED -ne 0 ]; then
  ...
  # Debug; dropping the rest would report Integration as having vanished on the next PHASE.
  {
    ...
    echo "#     read. It compares all sixteen from the next run on. If rows ever go missing again, that"
    echo "#     coverage silently narrows again: READ THE \"suite total(s) checked\" NUMBER, not just the word ok."
    echo "#"
