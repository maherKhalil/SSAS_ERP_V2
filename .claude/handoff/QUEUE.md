...
| # | item | status | detail |
|---|---|---|---|
| 123 | **Put the message in the problem document's `detail` — gated on one safety check.** RFC 7807 has the field, **40 call sites pass none**, and the thing that would normally make it dangerous is measured absent: **every message is a literal, so no runtime value can leak.** If `detail` carries the message, the 129 collapse costs machine-readability and not information. ⚠ **GATE, NOT OPTIONAL: for every wire code with more than one domain code behind it, establish whether exposing the messages reveals a distinction the collapse exists to HIDE.** `AccountActionToken.Invalid`'s five raises — one shared constant leaks nothing, five distinct constants hand an attacker an oracle over which invitation tokens exist. Same for `AccessTokenIssuer`. **Report the split before changing anything; where a group is unsafe, exclude it with the reason written at the exclusion.** | **open — ruled** | BOARD 2026-08-30 |
| 121 | **Six ambiguous error codes — establish who the caller is and whether it branches, then split only where it does.** `Department/Employee/Position.InvalidPagination` ×3 (*"page number **or** page size"* — **a client fixing the wrong one retries and fails again**), `AccountActionToken.Invalid` (*"invalid or unavailable"*, two remedies), `CompanyAccessErrors.AssignmentInvalid` (three-condition guard, one code), `UserEmployeeLink.Invalid`. ⚠ **Where the caller is external and could act differently, split the code — that is Principle 18's mechanism (1) under-applied, not a missing field.** Where it is an internal guard clause branching on nothing, **leave it and say so.** `Persistence.UniqueConstraint` is out of scope — `DEC-DEP-0027` rules it and that ruling stands. | **open — ruled** | BOARD 2026-08-30 |
