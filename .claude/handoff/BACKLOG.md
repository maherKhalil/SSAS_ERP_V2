...
| B12 | **A dead mapper arm in Payroll.** `PayElement.SetOvertimeTier` has **no production caller — only tests** — so `Payroll.PayElementOvertimeTierInvalid` cannot be raised from any request, **and the Payroll mapper carries an arm for a code nothing can produce.** Harmless today. **Establish whether the method itself should exist before removing anything** — a method with only test callers is either dead code or an unfinished capability, and the mapper arm is a symptom either way. | Found during PR #374 and deliberately not touched at the end of a session. |
