...
| 101 | **`PersistenceArchitectureTests` — the guard proven to pass when it measures nothing.** **Convert the two rules reflection answers exactly** — *"Domain and Application remain EF-free"* is an assembly-REFERENCE question, and *"no `IQueryable` on Application boundaries"* is a question about public method SIGNATURES — because an assembly that fails to load throws where a file walk that finds nothing returns empty and reads as success. **Floor whatever stays a file walk. Red first: reproduce the path rename, show the suite going green, then show it reddening after the fix.** That plant is the deliverable. | **open — ruled** | BOARD 2026-08-30 |
| 102 | **The other three with neither floor nor recorded plant:** `AuthenticationMilestone`, `AuthenticationSession`, `Localization`. All three assert an absence. **Floors plus plants; convert only where reflection is the natural question.** | **open** | BOARD 2026-08-30 |
