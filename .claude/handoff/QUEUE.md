...
| # | item | status | detail |
|---|---|---|---|
| **200** | ⚠ **RESTORE THE RATIONALE YOU RECOVERED — IT GUARDS A DELIBERATE EXCEPTION A FUTURE SWEEP WOULD *FIX* INTO A DEFECT.** `SqlServerBackupEvidence.cs:91` and `:156` carry bare `SqlDbType.Char, 1` beside `NVarChar` parameters, **and the reason exists nowhere in the repository** — that it compares against `msdb.dbo.backupset.type`, a Microsoft-owned system column that IS `char(1)`, so `NVarChar` would force an implicit conversion **of the column side**. **Put it at BOTH sites, quoted from the branch, and say where it was recovered from — a rationale whose provenance is a deleted branch should carry that fact.** ⚠ **AND MAKE IT ADDRESS THE SWEEP THAT WOULD BREAK IT: name `Every_persisted_string_in_the_tenant_model_is_unicode` and say why this is outside it** — the guard covers the TENANT MODEL and these are ADO parameters against a system database, **so a reader must not conclude the guard was evaded.** **Comment only, no behaviour change. Gated, so `DEC-L-007` applies.** | **open — ruled** | BOARD 2026-08-31 |
| **201** | **APPLY 198's METHOD TO THE ~200 `agent/*` BRANCHES — THE SCOPE YOU NAMED.** This repository has **280 branches** and 38 were examined; *only six codex branches carry anything* says nothing about the rest. ⚠ **USE THE CORRECTED INSTRUMENT, NOT THE ONE THAT MISLED US TWICE: compare CONTENT against `origin/ClaudeBranch` BY BASENAME — not reachability from `origin/main`, which is 925 commits behind, and not by PATH, which cannot distinguish DELETED from MOVED.** **Expected yield is LOW — these are this loop's own PR branches and they merged — and a near-empty answer is a good one. SAY SO IF THAT IS WHAT YOU FIND; I am not asking you to produce a finding.** ⚠ **ONE KNOWN EXCEPTION TO CHECK FIRST: `agent/T-072-spec-and-authorization-model`, whose content was landed DIRECTLY and whose PR was CLOSED — the branch is left in place on purpose, so it should read as *content present, branch retained deliberately* rather than as an orphan.** **If it does not, the method is wrong before it is trusted on 200 branches.** **MEASURE ONLY: no pushing, no deletion.** | **open — ruled** | BOARD 2026-08-31 |
