...
| **177** | ⚠⚠ **ENUMERATE FIRST, THEN GUARD: A UNIQUE INDEX OVER A NULLABLE COLUMN MUST CARRY A NULL FILTER.** Your `HasFilter` example is the acceptance criterion, and it must run under **TASK** — that is the whole point, since no TASK suite materialises a schema. ⚠ **BEFORE WRITING THE GUARD: 62 `IsUnique()` declarations exist and 17 carry a `HasFilter`, SO 45 DO NOT. For each of the 45, say whether every column in the index is non-nullable.** **If any is over a nullable column, that is a LIVE DATA DEFECT and it outranks the guard — report it and stop before building anything.** A guard written first would be shaped to pass over the existing failure. ⚠ **NULLABILITY IS THE HARD PART AND IT IS NOT THE C# TYPE: a `string` property may be configured `IsRequired()`, and a `Guid?` may be `.IsRequired()` too. Read the EF model's nullability, not the CLR type, and say which source you used.** **Plant it: delete the `HasFilter` from `EmployeeConfiguration` and the guard must redden and nothing else should.** ⚠ **And give it a matcher control — a guard that finds zero unique indexes passes.** | **open — ruled** | BOARD 2026-08-31 |
