...
| # | item | status | detail |
|---|---|---|---|
| 133 | **A STRUCTURAL sweep of the 129, replacing the keyword classifier that could only find shapes somebody anticipated.** 132 established the Class-A signature: **those codes are raised inside a domain VALUE OBJECT, which represents exactly one input by construction.** ⚠ **That is a structural predicate, not a vocabulary one — so ask it of all 129: which are raised inside a value object or entity rather than in a handler?** This answers three open questions in one pass: **Class B (4 range codes — do they carry TWO fields?), Class E (2 inactive-reference codes — field plus a reason?), and the 101 that matched no keyword and are UNEXAMINED, NOT CLEARED.** ⚠ **The "(b) serves 25 of 28" claim assumed B and E share this structure and nobody has looked — that assumption carries no weight until this runs.** Report the split; propose nothing. | **open — ruled** | BOARD 2026-08-30 |
