...
| # | item | status | detail |
|---|---|---|---|
| 122 | **Does the problem document carry the domain MESSAGE, or only the code?** ⚠ **This one measurement bounds the whole 129-code question.** Every `Error` has a literal message and 21 name several conditions in prose. **If the message reaches the response body, the `request.invalid` collapse costs machine-readability and not information** — a human debugging sees the detail even though a switch statement sees one code. **If only the code survives, the collapse is genuine information loss and the 129 is a real backlog.** One measurement against a live endpoint; it changes the size of the problem by an order of magnitude. | **open — ruled** | BOARD 2026-08-30 |
