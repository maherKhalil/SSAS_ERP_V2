...
| # | item | status | detail |
|---|---|---|---|
| 124 | **Add a known-positive control to the eight bans that have a floor and no matcher control.** `AuthenticationMilestone:78` and `:156`, `DepartmentApi:33` and `:61`, `PlatformSupportAuthenticationSurface:70`, `TenantCutoverFreeze:107`, plus **both guards written on 2026-08-30** — `ProblemDetailVisibility:120` and `SharedErrorMessage:51`. ⚠ **A floor sees a WALK collapse and cannot see a MATCHER collapse, because for a ban the matcher failing and the ban succeeding are the same observation.** Assert the pattern still matches a string it must match — **one line each, and strictly stronger than a plant: a plant proves the matcher worked when somebody checked; a control proves it on every run.** | **open — ruled** | BOARD 2026-08-30 |
