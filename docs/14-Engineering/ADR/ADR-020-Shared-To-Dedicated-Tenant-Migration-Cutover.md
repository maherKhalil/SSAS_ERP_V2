...
| 1.7 | 2026-09-06 | Architect window (`ssas-erp-v2-20`) | ⚠⚠ **THIS ENTRY IS NOT HOUSEKEEPING, AND IT SHOULD NOT BE READ BESIDE THE CORRECTIONS MADE TO `ADR-026` AND `ADR-027` THE SAME DAY. IT RECORDS A LIVE DIVERGENCE BETWEEN THIS ACCEPTED DECISION AND THE SHIPPED SYSTEM.** The line *"Implementation decision pending: cache technology, TTL, and the invalidation transport"* is resolved — `TenantRoutingMemoryCache` is a process-local `ConcurrentDictionary`, **no TTL** (*"unbounded by design in V1"*), and **no invalidation transport**: `Invalidate` is a local `TryRemove` whose own comment states *"THIS PROCESS ONLY… other instances converge through version validation, not through this call."* ***That is the divergence. The "Resolver cache" section states that BOTH mechanisms are required and rejects an earlier draft permitting either, because a node starting, partitioned or restarting can miss a broadcast — and the product implements no broadcast at all***, on the argument (recorded in the code, not here) that coherence stops being a correctness concern once the version is checked on every read. **That argument may be right; it is nonetheless this ADR's stated requirement being set aside, and the ADR has never said so.** ⚠ **What neither document answers: how quickly a node converges when it never receives a signal. The version check buys correctness, not convergence time — and this ADR itself classed the broadcast as the *performance* mechanism, so what was declined is unmeasured rather than free. Raised for the owner; no decision is changed by this entry.** |
