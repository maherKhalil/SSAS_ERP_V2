...
public static class BranchErrors
{
  ...
  // ---- THE SANCTIONED TRANSFER CHANNEL's errors live in SSAS.BuildingBlocks.Tenancy.Branches
  // (BranchTransferErrors), beside the contracts that return them.
  //
  // They were moved there in FP-006C3-pre because a business module opening the channel has to be able to
  // name what came back, and ADR-012 forbids it reaching into Platform's Domain to do so. The codes are
  // unchanged, so nothing a caller matches on has moved.
}
