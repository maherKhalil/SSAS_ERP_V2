...
| # | item | status | detail |
|---|---|---|---|
| **198** | ⚠⚠ **THE `codex/*` BRANCHES — CARRIED AS A STANDING OPEN ITEM ALL SESSION AND NEVER MEASURED. I HAVE STARTED IT AND THE NUMBERS ALREADY CONTRADICT THE STANDING NOTE.** **There are 38, not 13. THIRTEEN are local-only, which is where that figure came from — but only SIX carry any commit not already in `origin/main`, so THIRTY-TWO are branches over content that already shipped.** **The six: `architecture-tenant-storage` (1), `chain-test` (4), `fp-005-milestone-1` (1), `fp-007-department-analysis` (1), `post-phase-e-hardening-h1` (1), `sqlserver-unicode-enforcement` (1).** ⚠ **FOR EACH OF THE SIX: what is the commit, is its content in `main` by another route, and is anything on it a FIX THAT NEVER LANDED?** `sqlserver-unicode-enforcement` is the one I would read first — this repository has a live unicode persistence guard, and an orphaned commit there would be a real defect rather than an abandoned experiment. ⚠⚠ **MEASURE ONLY. DO NOT PUSH ANY BRANCH — THIS REPOSITORY IS PUBLIC AND PUSHING UNREVIEWED WORK IS PUBLICATION, NOT BACKUP. DO NOT DELETE ANY BRANCH — DELETION IS DESTRUCTIVE AND THE OWNER'S.** **Report what is on them and what would be lost; the disposal is a decision, not a measurement.** | **open — ruled** | BOARD 2026-08-31 |
| **199** | **WHAT WERE `Performance.Tests` AND `UI.Tests` FOR?** (was `B17`, downgraded when `test-baseline.txt` turned out to name both as deliberate empty scaffolds whose *absence is correct*.) **Each is a `.csproj` with no source file.** **Read the history: when were they added, in what commit, alongside what — and does anything in the repository still reference them or plan to?** **A `.csproj` in the solution is a claim that something belongs there.** ⚠ **DO NOT DELETE THEM ON THIS BASIS. An empty project may be a placeholder someone intends to fill, and that intention lives outside the repository.** **Report what they were for; the disposal is the owner's or mine.** **Small, and it needs no gate.** | **open — ruled** | BOARD 2026-08-31 |
