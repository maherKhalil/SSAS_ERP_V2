...
| # | item | status | detail |
|---|---|---|---|
| 136 | **Enumerate domain mutators called ONLY from tests.** Each is either **dead code** or **an unfinished capability with a silent product consequence** — B12 proved the second kind exists: `PayElement.SetOvertimeTier` is the sole write path to a persisted, calculator-read property, has no production caller, **and four test files call it directly, each supplying the missing half itself, so every test of the feature passes while the feature is unreachable.** ⚠ **This is the check that would have caught the overtime defect years before a payroll question did.** Report the list and classify each; **propose no removals** — the trace is worth more than the tidiness. | **open — ruled** | BOARD 2026-08-30 |
