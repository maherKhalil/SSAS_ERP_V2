...
Whether the platform session reuses `AuthenticationAccount.SecurityVersion` plus `AuthenticationSession` revocation, or introduces a distinct platform-session security-version, is a Phase-3C implementation detail deferred here; the guaranteed behaviour above holds regardless. This ADR does **not** assign `PlatformSupportPrincipal` its own `SecurityVersion`.
...
- **Phase 4** — `PlatformPermissionAuthorizationHandler`, `RequirePlatformPermission`, DI/Host policy integration, and externally exposing platform-authority administration only when authorized by `Platform.Support.Administer`.
- **Phase 5** — Tenant management HTTP endpoints.
