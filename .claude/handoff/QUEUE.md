...
| # | item | status | detail |
|---|---|---|---|
| 147 | **Re-derive the Axis 1 capability decomposition and NAME every member of every bucket.** `NEXT-SESSION.md` records *"41 owner-gated, 15 already deferred, 9 our own doc errors, 2 real"* — ⚠ **and the two real ones are named nowhere.** Do not trust the 2: **the count is a day old and three items have shipped against Axis 1 since.** **State blind spots with the result** — a capability reachable only through a shared route, or satisfied by a **seeded migration rather than a handler**, will not look implemented to a naive scan (the 138 SQL-seed class). **Use a known positive:** anything already shipped against Axis 1 must be rediscovered as *implemented* before any *missing* answer is believed. **Report and classify; the 9 doc errors are the architect's to fix and must be named.** | **open — ruled** | BOARD 2026-08-30 |
