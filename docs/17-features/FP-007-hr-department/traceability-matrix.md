...
| `REQ-HR-0100` `employeeCount` on the department representation | `DEC-POS-0034` | `IEmployeeReadService.CountEmployeesByDepartmentAsync`, `DepartmentCompositionServices`, `DepartmentResponse.EmployeeCount` — **shipped 2026-08-22**, after the FP-008 Phase 4 reconciliation found it specified but never built | `D33`–`D37`; three scope-containment tests in `DepartmentApplicationSqlServerTests` |
| `FR-DEP-0110`/`FR-DEP-0111` employee-side department surface | `DEC-DEP-0018`, `DEC-DEP-0019` | `EmployeeDepartmentSummary` on `EmployeeDetail`/`EmployeeSummary` joined from `tenant.Departments`; `EmployeeDepartmentResponse` on both wire shapes; `departmentId` in the employee search allowlist — **shipped 2026-08-22**, after the as-built audit found both specified and never built | `A46`–`A51`; `D15`, `D16` in `EmployeeBoundarySqlServerTests` |
| Department refusal status codes | `DEC-DEP-0030` | `DepartmentApiErrorMapper` — state-conflict refusals answer `409`; the `422` in the drafted contract was superseded by the shipped behaviour | `D23`, `D24`, `D25`, `D29`; the mapper's own arms |
